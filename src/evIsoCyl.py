...
cylSample = np.array([cylinderGen(nList[i],yMax) for i in range(3)])
cylPtSample=np.array([np.full(len(cylSample[i]), 1.) for i in range(3)])

numPart = np.array([len(cylPtSample[i]) for i in range(3)])
